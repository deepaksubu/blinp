...
class CollectedData {
	...
	public CollectedData(int blockStart, int blockEnd) {
		this.seqnoList = new ArrayList<Long>();
	...
	public void add(long seqno, long unixTime, int blockID, PlumSampleMsg message) {
...
//				if (requestMsg.get_cmdID() == PlumCommands.PLUM_TIME.value()) {
//					int unixTime = (int) (System.currentTimeMillis() / 1000L);
//					requestMsg.set_unixTime(unixTime);
//					
//					System.out.print("time value = " + requestMsg.get_unixTime() + "\n");
//				}
//				try {
//					mote.send(requestMsg.get_addr(), requestMsg);
...
//			}
	...
	synchronized public void requestConfig(int addr, int sampleRate, int statusRate) {
		if (requestPending == false) {
			...
			System.out.print("Request already pending for node " + requestMsg.get_addr() + ". Please try again later.\n");
		}
	}
	...
	synchronized public void requestSamples(int addr) {
		if (requestPending == false) {
			...
			m_read = true;
			collectedData = new CollectedData(requestMsg.get_blockStart(), requestMsg.get_blockEnd());
