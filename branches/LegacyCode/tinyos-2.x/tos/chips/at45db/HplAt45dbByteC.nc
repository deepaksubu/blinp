...
implementation 
{
  ...
  enum {
    ...
    P_START,
    P_STOP,
  } m_power_state = P_NONE;

  uint8_t status = P_OFF;
  uint8_t flashCmd[9];
  ...
  event void Resource.granted() {
    int power_state = m_power_state;
    m_power_state = P_NONE;
    if ( power_state == P_START ) {
      if ( status == P_OFF ) {
        call HplAt45dbByte.select();
        call FlashSpi.write(AT45_C_RESUME);
        call HplAt45dbByte.deselect();
        call BusyWait.wait(35);
      }
      call Resource.release();
      signal SplitControl.startDone(SUCCESS);
      status = P_IDLE;
      return;
    } else if ( power_state == P_STOP ) {
      call HplAt45dbByte.select();
      call FlashSpi.write(AT45_C_DEEP_SLEEP);
      call HplAt45dbByte.deselect();
      call Resource.release();
      call BusyWait.wait(3);
