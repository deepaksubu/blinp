...
public class PlumSensingApp implements MessageListener {
	...
	Queue<Integer> timeSetQueue = new LinkedList<Integer>();
	int timeSyncCounter;
			...
			if (m_scan == true) {
				...
				if (receivedMsg.get_sampleRate() < 60
					...
					if (Math.abs(receivedMsg.get_last_unixTime()
						...
						// Replace the below code by a queue
						// If received packet sender is at the top then send the
						// time
						if (timeSetQueue.peek()==null){
							this.timeSetQueue.add(receivedMsg.get_sender());
							System.out.println("Added "+receivedMsg.get_sender()+" to time queue");
						}
						if (this.timeSetQueue.peek() == receivedMsg
								.get_sender()) {
							
							if (timeSyncCounter==0){
								timeSyncCounter=4;
							}
							if (timeSyncCounter==4){
							...
							// else add it to the queue if it is not already
							// added
							if (!(this.timeSetQueue.contains(receivedMsg
									.get_sender()))) {
								this.timeSetQueue.add(receivedMsg.get_sender());
							}
						}
						...
						if (this.timeSetQueue.peek() == receivedMsg
								.get_sender()) {
							timeSetSender = this.timeSetQueue.poll();
							timeSyncCounter=4;
							System.out.println("Time is set for the plum "+timeSetSender);
						}
					}
				} else {
