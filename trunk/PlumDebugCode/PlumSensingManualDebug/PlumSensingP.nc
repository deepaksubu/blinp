...
#define N_SENSORS 1	
	...
	nx_plum_sample_t empty_sample_msg;
	nx_plum_send_msg_t plum_send_msg;
	...
	event void Boot.booted() {		
		...
		empty_sample_msg.seqno = 0;
		empty_sample_msg.lTime = 0;
		empty_sample_msg.unixTime = 0;
		empty_sample_msg.sender = 0;
		empty_sample_msg.sampleRate = 0;
		empty_sample_msg.statusRate = 0;		
		empty_sample_msg.intvol = 0;
		empty_sample_msg.blockID = 0;
	...
	void initSendMsg() {
		...
		for (i = 0 ; i < SEND_MESSAGE_SIZE ; i++) {
			memcpy(&(plum_send_msg.sample[i]), &empty_sample_msg, sizeof(nx_plum_sample_t));
		}
	}
...
//		if (call ReadPIR.read() == 1) {
	...
	task void sendSample() {		
		...
		if (radioBusy == FALSE) {
			dataTransfer = TRUE;
			while(sampleCounter < SEND_MESSAGE_SIZE) {
				if ((radioBufferTail + sampleCounter) % SAMPLE_RADIO_BUFFER_SIZE != radioCurrentBuffer) {
					copyToNx(&(plum_send_msg.sample[sampleCounter]), &(radioSampleBuffer[(radioBufferTail + sampleCounter) % SAMPLE_RADIO_BUFFER_SIZE]));
				}
				else {
					memcpy(&(plum_send_msg.sample[sampleCounter]), &empty_sample_msg, sizeof(nx_plum_sample_t));
				}
			}

			memcpy(call RadioSampleSend.getPayload(&sample_msg, sizeof(nx_plum_sample_t) * SEND_MESSAGE_SIZE), &plum_send_msg, sizeof(nx_plum_send_msg_t));
			
			if (serialSend == TRUE) {
				serialBusy = TRUE;
				err = call SerialSampleSend.send(BASE_STATION_ADDR, &sample_msg, sizeof(nx_plum_send_msg_t));
			} else {				
				radioBusy = TRUE;
				err = call RadioSampleSend.send(BASE_STATION_ADDR, &sample_msg, sizeof(nx_plum_send_msg_t));
			}
	...
	event void SerialSampleSend.sendDone(message_t* msg, error_t error) {
		...
		if (error == SUCCESS) {		
			for (sampleCounter = 0 ; sampleCounter < SEND_MESSAGE_SIZE ; sampleCounter++) {
//				if ((((nx_plum_send_msg_t*) (call SerialSampleSend.getPayload(msg, sizeof(nx_plum_send_msg_t))))->sample[sampleCounter])->sampleRate > 0) {		
				if ((((nx_plum_send_msg_t*) (call SerialSampleSend.getPayload(msg, sizeof(nx_plum_send_msg_t))))->sample[sampleCounter]).sampleRate > 0) {		
	...
	event void RadioSampleSend.sendDone(message_t* msg, error_t error) {
		...
		if (error == SUCCESS) {		
			for (sampleCounter = 0 ; sampleCounter < SEND_MESSAGE_SIZE ; sampleCounter++) {
				if ((((nx_plum_send_msg_t*) (call RadioSampleSend.getPayload(msg, sizeof(nx_plum_send_msg_t))))->sample[sampleCounter]).sampleRate > 0) {							
					radioBufferTail = (radioBufferTail + 1) % SAMPLE_RADIO_BUFFER_SIZE;
				}
				else {
					moreSamples = FALSE;
